export default function handler(req, res) {
  ...
  const sendMeta = (to, delayMs = 0) => {
    ...
    res.end(
      `<!doctype html><meta http-equiv="refresh" content="${delayMs}; url=${absolute(to)}"><title>Meta</title>`
    );
  };
  ...
  const sendJs = (to, delayMs = 100) => {
    ...
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.end(
      `<!doctype html><script>setTimeout(() => location.replace('${absolute(to)}'), ${delayMs})</script><title>JS</title>`
    );
  };
  ...
  switch (type) {
    ...
    case 'target':
      ...
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.end(
        '<h1>Redirect Test Target</h1><p>You have successfully reached the target page!</p>'
      );
      return;
