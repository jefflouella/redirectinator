export default function handler(req, res) {
  ...
  const sendMeta = to => {
    ...
    res.end(
      `<!doctype html><meta http-equiv="refresh" content="0; url=${absolute(to)}"><title>Meta</title>`
    );
  };
  ...
  const sendJs = to => {
    ...
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.end(
      `<!doctype html><script>location.replace('${absolute(to)}')</script><title>JS</title>`
    );
  };
  ...
  switch (slug) {
    ...
    // Two hop HTTP -> HTTP
    case '301-2':
      return redirect(301, '302-1');
    case '302-2':
      return redirect(302, '307-1');
    case '307-2':
      return redirect(307, '308-1');
    case '308-2':
      return redirect(308, '301-1');

    // Mixed two hop
    case 'meta-to-301':
      return sendMeta('301-1');
    case 'js-to-302':
      return sendJs('302-1');

    // Six hops
    case 'six-http':
      return redirect(301, 'a1');
    case 'a1':
      return redirect(302, 'a2');
    case 'a2':
      return redirect(307, 'a3');
    case 'a3':
      return redirect(308, 'a4');
    case 'a4':
      return redirect(301, 'a5');
    case 'a5':
      return redirect(302, target);
    ...
    case 'loop-b':
      return redirect(302, 'loop-start');

    // Messy combinations
    case 'messy-1':
      return redirect(307, 'meta-to-301');
    case 'messy-2':
      return sendJs('six-http');
    case 'messy-3':
      return redirect(301, 'js-to-302');
    case 'messy-4':
      return sendMeta('308-2');
    case 'messy-5':
      return redirect(302, 'loop-start');
