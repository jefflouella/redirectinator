...
class RedirectDetector {
   ...
   */
  detectMetaRefresh() {
    console.log('🔍 detectMetaRefresh called, checking for meta refresh tags...');
    ...
    if (metaRefresh) {
      ...
      if (content) {
        ...
        if (urlPart) {
          ...
          console.log('🔍 Parsed meta refresh - delay:', delay, 'targetUrl:', targetUrl);
          ...
          console.log('✅ Meta refresh detected and stored:', this.metaRefresh);
        } else {
      ...
      if (window.location.href !== this.originalUrl) {
        console.log('🔍 Page has already redirected from', this.originalUrl, 'to', window.location.href);
        console.log('🔍 This suggests the meta refresh or JavaScript redirect already executed');
      }
    }
  }
    ...
    try {
      ...
      window.addEventListener('hashchange', function(event) {
        if (event.oldURL !== event.newURL) {
          ...
          self.javascriptRedirects.push(redirect);
          console.log('JavaScript redirect detected (hash change):', redirect);
        }
      });
      ...
      setInterval(() => {
        ...
        if (currentLocation !== lastLocation && currentLocation !== self.originalUrl) {
          const redirect = {
            type: 'javascript',
          ...
          console.log('JavaScript redirect detected (location change):', redirect);
        }
      }, 1000);
  ...
  overrideRedirectMethods() {
    ...
    try {
      ...
      try {
        ...
        if (originalHrefDescriptor && originalHrefDescriptor.set && originalHrefDescriptor.configurable) {
          Object.defineProperty(window.location, 'href', {
            set: function(value) {
              if (value !== window.location.href) {
                ...
                self.javascriptRedirects.push(redirect);
                console.log('JavaScript redirect detected (href setter):', redirect);
              }
        ...
        console.log('⚠️ Could not override location.href:', hrefError.message);
      }
      ...
      try {
        ...
        if (originalReplace && typeof originalReplace === 'function') {
          window.location.replace = function(url) {
            if (url !== window.location.href) {
              ...
              self.javascriptRedirects.push(redirect);
              console.log('JavaScript redirect detected (replace):', redirect);
            }
        ...
        console.log('⚠️ Could not override location.replace:', replaceError.message);
      }
      ...
      try {
        ...
        console.log('⚠️ Could not override location.assign:', assignError.message);
      }

      console.log('Redirectinator Advanced: JavaScript redirect method overrides completed');
    } catch (error) {
      console.warn('Redirectinator Advanced: Error setting up method overrides:', error);
    }
  }
  ...
  getAnalysisResults() {
    ...
    console.log('  - javascriptRedirects count:', this.javascriptRedirects.length);
    console.log('  - metaRefreshRedirects count:', (this.metaRefreshRedirects || []).length);
  ...
  checkInjectedScriptRedirects() {
    try {
      ...
      if (window.redirectinatorRedirects && Array.isArray(window.redirectinatorRedirects)) {
        console.log('🔍 Found redirects from injected script:', window.redirectinatorRedirects);

        // Add any new redirects that we haven't seen before
        window.redirectinatorRedirects.forEach((redirect) => {
          const existingRedirect = this.javascriptRedirects.find(r =>
            r.from === redirect.from && r.to === redirect.to && r.method === redirect.method
          );
      ...
      console.log('🔍 Error checking injected script redirects:', error.message);
    }
  }
}
  ...
  try {
    ...
    window.addEventListener('message', function(event) {
      ...
      if (event.data?.type === 'REDIRECTINATOR_REDIRECT_DETECTED') {
        console.log('🔍 Received redirect detection from monitoring script:', event.data.redirect);
      ...
      if (event.data?.type === 'REDIRECTINATOR_META_REFRESH_DETECTED') {
        console.log('🔍 Received meta refresh detection from monitoring script:', event.data.metaRefresh);

        if (redirectDetector) {
          redirectDetector.metaRefresh = event.data.metaRefresh;
          console.log('✅ Meta refresh updated in detector:', event.data.metaRefresh);
        }
      }
    });
    ...
    try {
      console.log('🔍 Attempting to inject monitoring script with CSP workaround...');
      ...
      console.log('🔍 Monitoring script content created, length:', monitoringScript.length);
      ...
      script.onerror = function(error) {
        console.log('⚠️ Data URI injection failed, trying alternative approach...');
        ...
        try {
          ...
          if (document.head) {
            ...
            console.log('✅ Monitoring script injected via function call (documentElement)');
          }
        } catch (fallbackError) {
          console.log('⚠️ Function call injection also failed:', fallbackError.message);
      ...
      try {
        ...
        const sendMessagePromise = new Promise((resolve, reject) => {
          function resultListener(message) {
            if (message && message.type === 'ANALYZE_URL_RESULT' && message.requestId === event.data.requestId) {
        ...
        console.log('🔍 Message sent to background script, waiting for ANALYZE_URL_RESULT...');
      } catch (error) {
