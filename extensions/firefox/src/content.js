...
if (
    ...
    monitorMetaRefreshChanges() {
      ...
      const checkForMetaRefreshRedirect = () => {
        if (window.location.href !== lastUrl) {
          console.log(
            '🔍 Location changed, checking for meta refresh execution'
          );
  ...
  function injectExtensionAPI() {
    try {
      // Inject a global object that the web app can detect
      window.redirectinatorExtension = {
        isAvailable: () => true,
        getVersion: () => '1.0.0-local',
      ...
      console.log('Redirectinator Advanced: Extension API injected');
  ...
  window.addEventListener('message', event => {
    ...
    if (event.data?.type === 'REDIRECTINATOR_REQUEST') {
      ...
      if (
        ...
        console.error(
          '❌ Browser runtime not available for background communication'
        );
        window.postMessage(
          {
      ...
      try {
          ...
          browser.runtime.sendMessage(
            ...
            response => {
              ...
              if (browser.runtime.lastError) {
                ...
                console.error(
                  '❌ Response is undefined - background script may have crashed'
                );
                reject(new Error('No response from background script'));
              } else {
                console.log(
                  '✅ Valid response received from background script'
                );
                resolve(response);
              }
            }
        ...
        console.log(
          '🔍 Message sent to background script, waiting for response...'
        );
      } catch (error) {
        console.error(
