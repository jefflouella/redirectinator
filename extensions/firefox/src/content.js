...
window.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'REDIRECTINATOR_PING') {
    ...
    try {
      const extensionId = typeof browser !== 'undefined' && browser.runtime ? browser.runtime.id : undefined;
      window.postMessage({
  ...
  if (event.data?.type === 'REDIRECTINATOR_REQUEST') {
    ...
    const tryCommunication = async () => {
      try {
        ...
        try {
          const port = browser.runtime.connect({ name: 'CONTENT_SCRIPT_ANALYZE' });
          ...
          port.onDisconnect.addListener(() => {
            ...
            if (browser.runtime.lastError) {
              console.error('❌ Port error:', browser.runtime.lastError);
              // Try fallback method
