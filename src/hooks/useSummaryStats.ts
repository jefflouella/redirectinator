...
export const useSummaryStats = (results: RedirectResult[]) => {
  const summaryStats = useMemo(() => {
    const stats: SummaryStats = {
      ...
      containsOnly301: results.filter(
        r => r.httpStatus.includes('301') && !r.httpStatus.includes('302')
      ).length,
      contains302: results.filter(r => r.httpStatus.includes('302')).length,
      contains4xx: results.filter(
        r => r.finalStatusCode >= 400 && r.finalStatusCode < 500
      ).length,
