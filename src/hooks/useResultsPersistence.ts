...
export const useResultsPersistence = ({ projectId }: UseResultsPersistenceProps) => {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      ...
      const processed = new Set(projectResults.map(result => result.startingUrl));
  ...
  const saveResults = useCallback(async (newResults: RedirectResult[]) => {
    if (!projectId) return;
  ...
  // Add single result
  const addResult = useCallback(async (result: RedirectResult) => {
    if (!projectId) return;
  ...
  const updateResult = useCallback(async (index: number, updatedResult: RedirectResult) => {
    if (!projectId) return;

    try {
      const newResults = results.map((result, i) => i === index ? updatedResult : result);
      await storageService.saveResults(newResults, projectId);
      setResults(newResults);
    } catch (error) {
  ...
  const getUnprocessedUrls = useCallback((urls: Array<{ startingUrl: string; targetRedirect: string }>) => {
    return urls.filter(url => !processedUrls.has(url.startingUrl));
  }, [processedUrls]);

  // Get progress statistics
  const getProgressStats = useCallback((totalUrls: number) => {
    const processed = processedUrls.size;
    const remaining = totalUrls - processed;
    const percentage = totalUrls > 0 ? Math.round((processed / totalUrls) * 100) : 0;
