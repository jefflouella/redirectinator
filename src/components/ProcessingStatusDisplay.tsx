    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className={`p-3 border rounded-lg ${
        ...
        <div className="flex items-center space-x-2">
          <div className={`animate-spin rounded-full h-4 w-4 border-b-2 ${
            processingStatus?.isStopping ? 'border-orange-600' : 'border-blue-600'
          }`}></div>
      ...
      <div className="mt-4 space-y-2">
        <div className={`text-sm p-2 rounded ${
          ...
          {processingStatus?.isStopping ? 'Stopping' : 'Processing'} batch {processingStatus?.currentBatch || 0} of{' '}
        ...
        {processingStatus?.isStopping && (
          <div className="text-sm text-orange-600 bg-orange-50 p-2 rounded">
            Stop request received. Processing will stop after the current batch completes.
          </div>
        )}
      </div>
    </div>
